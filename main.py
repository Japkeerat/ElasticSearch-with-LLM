...
def setup_phoenix_tracing(phoenix_endpoint: str = "http://localhost:6006") -> bool:
    ...
    if not PHOENIX_AVAILABLE:
        ...
        print("To enable: pip install arize-phoenix opentelemetry-api opentelemetry-sdk opentelemetry-exporter-otlp")
        return False
...
async def process_user_query(runner, session_service, query: str, user_id: str, app_name: str, logger: logging.Logger, tracer=None) -> None:
    """
    ...
    if tracer:
        with tracer.start_as_current_span(span_name) as span:
            ...
            await _process_query_internal(runner, session_service, query, user_id, app_name, logger, span)
    else:
        await _process_query_internal(runner, session_service, query, user_id, app_name, logger, None)


async def _process_query_internal(runner, session_service, query: str, user_id: str, app_name: str, logger: logging.Logger, span=None):
    ...
    try:
        ...
        if span:
            ...
            span.set_attribute("response.text", final_response_text[:500])  # Truncate for span
            if PHOENIX_AVAILABLE:
            ...
            try:
                ...
                if tracer:
                    with tracer.start_as_current_span("user_interaction") as interaction_span:
                        ...
                        await process_user_query(runner, session_service, user_input, USER_ID, APP_NAME, logger, tracer)
                else:
                    await process_user_query(runner, session_service, user_input, USER_ID, APP_NAME, logger, None)
