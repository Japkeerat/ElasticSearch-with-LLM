...
from llm_es_agent.tools.elasticsearch_tools import QueryGenerationTools
...
class GeneratedQuery(BaseModel):
    ...
    query_type: str = Field(description="Type of query: 'match', 'term', 'range', 'bool', 'aggregation', etc.")
...
class QueryMetadata(BaseModel):
    ...
    generation_method: str = Field(description="Method used: 'automatic', 'field_analysis', 'user_clarification', or 'error'")
    user_intent: str = Field(description="Interpreted user intent from the natural language query")
    field_mappings: Dict[str, str] = Field(description="Mapping of user terms to actual index fields")
...
class QueryValidation(BaseModel):
    ...
    syntax_valid: bool = Field(description="Whether the query has valid Elasticsearch DSL syntax")
    fields_exist: bool = Field(description="Whether all referenced fields exist in the index schema")
    query_safe: bool = Field(description="Whether the query is read-only and safe to execute")
    ready_for_execution: bool = Field(description="Whether the query is ready to be executed")


class QueryGenerationOutput(BaseModel):
    """Structured output for the Query Generation Agent."""
    generated_query: Optional[GeneratedQuery] = Field(description="The generated Elasticsearch query, or null if generation failed")
    target_index: str = Field(description="Name of the target index for the query")
    query_metadata: QueryMetadata = Field(description="Metadata about the query generation process")
    validation: QueryValidation = Field(description="Validation results for the generated query")
...
class QueryGenerationAgent:
    ...
    def _create_agent(self) -> LlmAgent:
        ...
        validate_fields_tool = FunctionTool(self.query_tools.validate_fields_against_schema)
    ...
    def _get_agent_instructions(self) -> str:
        ...
        """
        file_path = Path(__file__).parent.parent.parent / "prompts" / "query_generation_agent.txt"
...
if __name__ == "__main__":
    ...
    field_validation = query_agent.query_tools.validate_fields_against_schema(test_query, mock_schema)
